...
| Deploy (cloud) | Vercel (SPA), Render (Node backend) — see `vercel.json`, `render.yaml` |
...
2. **Render** — Deploy `backend/` using `render.yaml` (Node 20, health check `GET /health`).
...
- Optional: Razorpay, Cohere (rerank), ffmpeg (TTS OGG on VPS — included in `backend/Dockerfile`)
...
**Note:** `tenant.ts` may call the public API using `VITE_API_URL` on the **backend** env — set it to the same public API base URL on Render/Coolify.
...
| `backend/` | `npm run dev` | Express + workers (tsx watch) |
| `backend/` | `npm run build` / `npm start` | Compiled production server |
