...
const HeroSection = () => {
  ...
  return (
    <section className="relative pt-32 pb-40 overflow-hidden bg-gradient-to-b from-white to-[#F4F7F5] border-b border-[#EAEAEA]">
      <Helmet>
      ...
      {/* Optimized CSS-only background grid for maximum speed */}
      <div className="absolute inset-0 z-0 opacity-40 pointer-events-none">
        <div className="absolute inset-0" style={{ backgroundImage: 'radial-gradient(#002E23 2px, transparent 2px)', backgroundSize: '32px 32px', opacity: 0.15 }} />
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_transparent_10%,_#ffffff_70%)]"></div>
        <div className="absolute top-0 right-0 w-[800px] h-[800px] bg-gradient-to-b from-[#A7C7B9]/20 to-transparent rounded-full blur-3xl -translate-y-1/2 translate-x-1/3"></div>
      </div>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="text-left">
            ...
            <p className="hero-subtext opacity-0 text-lg md:text-xl text-[#4A6B5D] max-w-xl mb-12 font-medium leading-relaxed">
      ...
      <section className="py-32 bg-[#FAFAFA] border-y border-[#EAEAEA] overflow-hidden">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {[
              { icon: Database, title: "Vector RAG", desc: "We use pgvector to embed your PDFs and knowledge base. The AI strictly answers based on your context, eliminating hallucinations." },
              { icon: Activity, title: "Adaptive Routing", desc: "An ultra-fast LLM classifies user intent. If a user asks 'Cancel my order AND what are your hours?', it splits and handles both simultaneously." },
              { icon: Zap, title: "Zero-Latency Caching", desc: "We safely cache AI responses globally. If multiple users ask similar questions across different languages, they get instant answers with zero LLM cost." },
              { icon: Bot, title: "Self-Learning Miner", desc: "A nightly engine analyzes your chat logs to find high-frequency questions, automatically generating draft FAQs to continually smarten your bot." },
              { icon: ShoppingCart, title: "Commerce Sync", desc: "Natively connected to Shopify. Customers can check order status, browse catalogs, and get support entirely within WhatsApp." },
              { icon: MessageSquare, title: "Unified Inbox", desc: "When the AI can't answer, it escalates smoothly. Human agents step into a shared inbox with full chat history and context." }
            ].map((feature, idx) => (
              <motion.div 
                key={idx}
                initial={{ opacity: 0, y: 30 }}
                ...
                className="relative h-full bg-white rounded-3xl p-8 border border-[#EAEAEA] hover:border-[#002E23]/30 shadow-sm hover:shadow-2xl hover:shadow-[#002E23]/10 transition-all duration-300 group overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-[#002E23]/5 to-transparent rounded-bl-full opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <div className="w-14 h-14 bg-[#F4F7F5] rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 group-hover:bg-[#002E23] transition-all duration-300 shadow-sm">
                  <feature.icon className="w-6 h-6 text-[#002E23] group-hover:text-white transition-colors" />
                </div>
                <h3 className="text-2xl font-bold text-[#00221A] mb-4 relative z-10 group-hover:text-[#002E23] transition-colors">{feature.title}</h3>
                <p className="text-[#4A6B5D] leading-relaxed relative z-10">{feature.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
