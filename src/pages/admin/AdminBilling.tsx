    ...
    <div className="space-y-8 relative max-w-7xl mx-auto text-theme-text">
      <div className="border-b border-theme-border pb-4">
        <h1 className="text-3xl font-bold text-theme-text flex items-center tracking-tight">
          <CreditCard className="mr-3 h-8 w-8 text-brand-accent" />
          Billing & Subscriptions
        </h1>
        <p className="text-theme-text-muted mt-2 text-lg font-medium">Track platform revenue, active subscriptions, and renewal dates.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="p-6 bg-theme-surface border border-theme-border rounded flex items-center shadow-sm hover:shadow-md transition-shadow">
          <div className="p-4 bg-emerald-500/10 text-emerald-500 rounded mr-4 border border-emerald-500/20">
            <TrendingUp size={28} strokeWidth={2.5} />
          </div>
          <div>
            <div className="text-theme-text-muted text-xs font-bold uppercase tracking-wider mb-1">Total Verified MRR</div>
        ...
        <div className="p-6 bg-theme-surface border border-theme-border rounded flex items-center shadow-sm hover:shadow-md transition-shadow">
          ...
          <div>
            <div className="text-theme-text-muted text-xs font-bold uppercase tracking-wider mb-1">Active Subscriptions</div>
            <div className="text-3xl font-mono font-bold text-theme-text">
          ...
          <table className="w-full text-left border-collapse bg-theme-bg">
            <thead>
              <tr className="border-b border-theme-border bg-theme-surface/50">
                ...
                <th className="py-3 px-4 font-bold text-sm text-theme-text-muted uppercase tracking-wider border-r border-theme-border">Price (MRR)</th>
                ...
                <th className="py-3 px-4 font-bold text-sm text-theme-text-muted uppercase tracking-wider border-r border-theme-border">Next Billing</th>
            ...
            <tbody className="divide-y divide-slate-800">
              {tenants.map((t) => {
                ...
                return (
                  <tr key={t.id} className="hover:bg-theme-surface transition-colors">
                    <td className="py-3 px-4 border-r border-theme-border">
                      <div className="font-bold text-theme-text">{t.business_name}</div>
                      <div className="font-mono text-[10px] text-theme-text-muted">{t.id.substring(0,8)}</div>
                    </td>
                    <td className="py-3 px-4 border-r border-theme-border">
                      <span className="text-sm font-bold uppercase text-theme-text-muted">{sub?.plan || 'N/A'}</span>
                    </td>
                    <td className="py-3 px-4 border-r border-theme-border font-mono font-bold text-brand-accent">
                    ...
                    <td className="py-3 px-4 font-mono text-sm text-theme-text flex items-center border-r border-theme-border h-full min-h-[50px]">
                      <Calendar className="w-4 h-4 mr-2 text-theme-text-muted" />
                      {sub?.renewed_at ? (() => {
                    ...
                    <td className="py-3 px-4 text-center">
                      <button 
                        ...
                        className="p-2 text-theme-text-muted hover:text-amber-500 transition-colors rounded-full hover:bg-amber-500/10 inline-flex"
                      >
              ...
              {tenants.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-8 text-center text-theme-text-muted italic bg-theme-surface">No billing data found.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="p-8 bg-theme-surface border border-theme-border rounded w-full max-w-md relative">
            <div className="flex justify-between items-center mb-6 border-b border-theme-border pb-4">
              ...
              <button onClick={() => setEditingSub(null)} className="p-1 text-theme-text-muted hover:text-theme-text transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleSaveSub} className="space-y-5">
              <div>
                <label className="block text-sm font-bold text-theme-text-muted mb-2">Workspace</label>
                <input 
                  type="text" 
                  value={editingSub.business_name}
                  disabled
                  className="w-full bg-theme-bg border border-theme-border text-theme-text-muted rounded px-4 py-3 font-medium cursor-not-allowed"
                />
              </div>

              <div>
                <label className="block text-sm font-bold text-theme-text-muted mb-2">Plan / Tier</label>
                <select 
                  value={subForm.plan}
                  onChange={e => setSubForm({...subForm, plan: e.target.value})}
                  className="w-full bg-theme-bg border border-theme-border text-theme-text rounded px-4 py-3 focus:outline-none focus:border-amber-500 transition-all font-medium"
                >
              ...
              <div>
                <label className="block text-sm font-bold text-theme-text-muted mb-2">Monthly Price (INR)</label>
                <input 
                  type="number" 
                  min="0"
                  value={subForm.price_inr}
                  onChange={e => setSubForm({...subForm, price_inr: parseFloat(e.target.value) || 0})}
                  className="w-full bg-theme-bg border border-theme-border text-theme-text rounded px-4 py-3 focus:outline-none focus:border-amber-500 transition-all font-mono text-lg"
                  required
                />
              </div>

              <div>
                <label className="block text-sm font-bold text-theme-text-muted mb-2">Subscription Status</label>
                <select 
              ...
              <div className="pt-6 border-t border-theme-border flex justify-end gap-4 mt-6">
                ...
                <button
                  type="submit"
                  disabled={saving}
                  className="flex-1 px-6 py-3 bg-amber-600 hover:bg-amber-500 text-theme-text font-bold rounded transition-colors disabled:opacity-50 flex items-center justify-center shadow-md"
                >
