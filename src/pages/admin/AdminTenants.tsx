    ...
    <div className="space-y-8 relative max-w-7xl mx-auto text-theme-text">
      <div className="border-b border-theme-border pb-4">
        <h1 className="text-3xl font-bold text-theme-text flex items-center tracking-tight">
          <Building className="mr-3 h-8 w-8 text-brand-accent" />
          Tenant Directory
        </h1>
        <p className="text-theme-text-muted mt-2 text-lg font-medium">Manage workspaces, enforce quotas, and configure BSP settings.</p>
      </div>

      <div className="grid grid-cols-1 gap-8">
        <div className="p-6 bg-theme-surface border border-theme-border rounded shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold flex items-center text-theme-text">
              <Users className="mr-2 h-5 w-5 text-brand-accent" /> Workspaces
            </h2>
          </div>
          ...
          <div className="overflow-x-auto rounded border border-theme-border">
            <table className="w-full text-left border-collapse bg-theme-bg">
              ...
              <tbody className="divide-y divide-slate-800">
                {tenants.map((t) => {
                  ...
                  return (
                    <React.Fragment key={t.id}>
                      <tr 
                        className={cn(
                          "hover:bg-theme-surface transition-colors cursor-pointer",
                          expandedTenantId === t.id && "bg-theme-surface"
                        )}
                      ...
                      >
                        ...
                        <td className="py-3 px-4 border-r border-theme-border">
                          ...
                          <div className="w-full bg-theme-surface-hover h-1.5 rounded-full overflow-hidden">
                            <div 
                              className={cn(
                        ...
                        <td className="py-3 px-4 text-center action-dropdown-container relative">
                          <button 
                            ...
                            className="p-2 text-theme-text-muted hover:text-brand-accent transition-colors rounded-full hover:bg-indigo-500/10 inline-flex items-center justify-center"
                          >
                            <MoreVertical className="w-5 h-5" />
                          </button>
                          
                          {openDropdownId === t.id && (
                            <div className="absolute right-8 top-10 w-48 bg-theme-surface border border-theme-border rounded shadow-xl z-10 py-1">
                              <button 
                      ...
                      {expandedTenantId === t.id && (
                        <tr className="bg-theme-surface border-b border-theme-border">
                          <td colSpan={5} className="p-0">
                            <div className="p-6 bg-theme-bg/50 border-t border-theme-border grid grid-cols-1 md:grid-cols-3 gap-6 shadow-inner">
                              
                              {/* Team Overview */}
                              <div className="bg-theme-surface p-4 rounded border border-theme-border">
                ...
                {tenants.length === 0 && (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-theme-text-muted italic bg-theme-surface">No tenants found.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className="p-6 self-start bg-theme-surface border border-theme-border rounded">
          <h2 className="text-xl font-bold flex items-center text-theme-text mb-6">
          ...
          <form onSubmit={handleProvision} className="space-y-4">
            <div>
              ...
              <input 
                ...
                required 
                className="w-full px-4 py-3 border border-theme-border bg-theme-bg text-theme-text focus:border-indigo-500 focus:outline-none transition-colors rounded"
              />
            </div>
            ...
            <div>
              ...
              <select 
                ...
                className="w-full px-4 py-3 border border-theme-border bg-theme-bg text-theme-text focus:border-indigo-500 focus:outline-none transition-colors rounded font-bold"
              >
            ...
            <button 
              type="submit" 
              disabled={submitting}
              className="w-full mt-6 px-4 py-3 bg-indigo-600 text-theme-text font-bold tracking-wide hover:bg-indigo-500 disabled:opacity-50 transition-colors rounded shadow-md"
            >
      ...
      {quotaModalTenant && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="p-8 bg-theme-surface border border-theme-border rounded w-full max-w-sm relative">
            <h2 className="text-xl font-bold text-theme-text mb-2 flex items-center">
            ...
            <form onSubmit={handleUpdateQuota}>
              <div className="mb-6">
                ...
                <input 
                  ...
                  className="w-full px-4 py-3 border border-theme-border bg-theme-bg text-theme-text focus:border-indigo-500 focus:outline-none transition-colors rounded font-mono text-lg"
                />
              </div>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  className="flex-1 px-6 py-3 bg-amber-600 text-theme-text font-bold hover:bg-amber-500 disabled:opacity-50 rounded shadow-md transition-colors"
                >
      ...
      {bspModalTenantId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="p-8 bg-theme-surface border border-theme-border rounded w-full max-w-lg max-h-[90vh] overflow-y-auto relative">
            <div className="flex justify-between items-center mb-6 border-b border-theme-border pb-4">
              <h2 className="text-2xl font-bold text-theme-text">Configure BSP</h2>
            ...
            {loadingBsp ? (
              ...
              <form onSubmit={handleSaveBsp} className="p-6 space-y-5">
                <div>
                  <label className="block text-sm font-bold text-theme-text-muted mb-2">Provider</label>
                  <select 
                ...
                <div>
                  <label className="block text-sm font-bold text-theme-text-muted mb-2">WABA ID</label>
                  <input 
                    type="text" 
                    value={bspForm.waba_id}
                    onChange={e => setBspForm(prev => ({ ...prev, waba_id: e.target.value }))}
                    className="w-full bg-theme-bg border border-theme-border text-theme-text rounded px-4 py-3 focus:outline-none focus:border-indigo-500 transition-all font-mono text-sm"
                    required
                ...
                <div>
                  <label className="block text-sm font-bold text-theme-text-muted mb-2">Phone Number ID</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-sm font-bold text-theme-text-muted mb-2">
                    ...
                    <span className="block text-xs font-normal text-theme-text-muted mt-1">Leave blank to keep existing. Will be encrypted at rest.</span>
                  </label>
                  <input 
                    type="password" 
                ...
                {bspConfig?.webhook_verify_token && bspForm.provider !== 'meta' && (
                  <div className="bg-theme-bg p-4 border border-theme-border rounded mt-4">
                    <label className="block text-xs font-bold uppercase text-theme-text-muted mb-2">Webhook Verify Token (Gupshup)</label>
                    <code className="text-brand-accent font-mono text-sm break-all font-bold">{bspConfig.webhook_verify_token}</code>
                    <p className="text-xs text-theme-text-muted mt-2 font-medium">Configure this token in your Gupshup dashboard.</p>
                  </div>
                )}

                {bspForm.provider === 'meta' && (
                  <div className="bg-theme-bg p-4 border border-blue-800 rounded mt-4">
                    <label className="block text-xs font-bold uppercase text-blue-400 mb-2">Meta Webhook Setup</label>
                    <p className="text-xs text-theme-text-muted font-medium">Meta uses a global <code className="text-blue-400">META_VERIFY_TOKEN</code> env var on the backend. Do not use the per-tenant token.</p>
                  </div>
                )}
                ...
                <div className="pt-6 mt-6 border-t border-theme-border flex gap-4">
                  ...
                  <button 
                    type="submit" 
                    disabled={savingBsp}
                    className="flex-1 px-6 py-3 bg-indigo-600 text-theme-text font-bold hover:bg-indigo-500 disabled:opacity-50 rounded shadow-md transition-colors"
                  >
